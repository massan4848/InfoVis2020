function Isosurfaces( volume, isovalue ){
        ...
        for ( var y = 0; y < volume.resolution.y - 1; y++ ){
            for ( var x = 0; x < volume.resolution.x - 1; x++ ){
                ...
                for ( var j = 0; lut.edgeID[index][j] != -1; j += 3 ){
                    ...
                    // var sc0 = volume.values[indices[0]][0];
                    // var sc1 = volume.values[indices[1]][0];
                    // var sc2 = volume.values[indices[2]][0];
                    // var sc3 = volume.values[indices[3]][0];
                    // var sc4 = volume.values[indices[4]][0];
                    // var sc5 = volume.values[indices[5]][0];
                    ...
                    // var I0 = Math.round( normalized( (sc0+sc1)*0.5, smin, smax ) * 255 );
                    // var I1 = Math.round( normalized( (sc2+sc3)*0.5, smin, smax ) * 255 );
                    // var I2 = Math.round( normalized( (sc4+sc5)*0.5, smin, smax ) * 255 );
                    // var C0 = new THREE.Color().setHex( cmap[ I0 ][1] );
                    // var C1 = new THREE.Color().setHex( cmap[ I1 ][1] );
                    // var C2 = new THREE.Color().setHex( cmap[ I2 ][1] );
                    // geometry.faces[cnt].vertexColors.push( C0 );
                    // geometry.faces[cnt].vertexColors.push( C1 );
                    // geometry.faces[cnt].vertexColors.push( C2 );
                }
            }
            cell_index++;
        }
    ...
    function table_index( indices ){
        var s = [];
        for(var i = 0;i<8;i++){
            s.push(volume.values[indices[i]][0]);
        }
        var index = 0;
        var li = [1,2,4,8,16,32,64,128];
        for(var i = 0;i<8;i++){
            if ( s[i] > isovalue ) { index |= li[i]; }
        }
        return index;
    }
