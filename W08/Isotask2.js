function Isosurfaces( volume, isovalue ){
        ...
        for ( var y = 0; y < volume.resolution.y - 1; y++ ){
            for ( var x = 0; x < volume.resolution.x - 1; x++ ){
                ...
                for ( var j = 0; lut.edgeID[index][j] != -1; j += 3 ){
                    ...
                    // var i0 = (v0.y*volume.resolution.x)+(v0.z*volume.resolution.y)+(v0.x*volume.resolution.z);
                    // var s0 = volume.values[i0][0];        
                    // var i1 = (v1.y*volume.resolution.x)+(v1.z*volume.resolution.y)+(v1.x*volume.resolution.z);
                    // var s1 = volume.values[i1][0];
                    // var i2 = (v2.y*volume.resolution.x)+(v2.z*volume.resolution.y)+(v2.x*volume.resolution.z);
                    // var s2 = volume.values[i2][0];        
                    // var i3 = (v3.y*volume.resolution.x)+(v3.z*volume.resolution.y)+(v3.x*volume.resolution.z);
                    // var s3 = volume.values[i3][0];
                    // var i4 = (v4.y*volume.resolution.x)+(v4.z*volume.resolution.y)+(v4.x*volume.resolution.z);
                    // var s4 = volume.values[i4][0];        
                    // var i5 = (v5.y*volume.resolution.x)+(v5.z*volume.resolution.y)+(v5.x*volume.resolution.z);
                    // var s5 = volume.values[i5][0];
                    var i0 = (v0.y*volume.resolution.x*volume.resolution.z)+(v0.z*volume.resolution.x*volume.resolution.y)+(v0.x*volume.resolution.y*volume.resolution.z);
                    var s0 = volume.values[i0][0];        
                    var i1 = (v1.y*volume.resolution.x*volume.resolution.z)+(v1.z*volume.resolution.x*volume.resolution.y)+(v1.x*volume.resolution.y*volume.resolution.z);
                    var s1 = volume.values[i1][0];
                    var i2 = (v2.y*volume.resolution.x*volume.resolution.z)+(v2.z*volume.resolution.x*volume.resolution.y)+(v2.x*volume.resolution.y*volume.resolution.z);
                    var s2 = volume.values[i2][0];        
                    var i3 = (v3.y*volume.resolution.x*volume.resolution.z)+(v3.z*volume.resolution.x*volume.resolution.y)+(v3.x*volume.resolution.y*volume.resolution.z);
                    var s3 = volume.values[i3][0];
                    var i4 = (v4.y*volume.resolution.x*volume.resolution.z)+(v4.z*volume.resolution.x*volume.resolution.y)+(v4.x*volume.resolution.y*volume.resolution.z);
                    var s4 = volume.values[i4][0];        
                    var i5 = (v5.y*volume.resolution.x*volume.resolution.z)+(v5.z*volume.resolution.x*volume.resolution.y)+(v5.x*volume.resolution.y*volume.resolution.z);
                    var s5 = volume.values[i5][0];
    ...
    function interpolated_vertex(v0, v1, s, s0, s1){
        ...
        // return new THREE.Vector3().addVectors( (s1-s)*v0, (s-s0)*v1 ).divideScalar(s1-s0);    
    }
}
